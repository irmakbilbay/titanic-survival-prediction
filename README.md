...
Veri Keşfi (EDA): Cinsiyet ve bilet sınıfının hayatta kalma üzerindeki kritik etkisinin görselleştirilmesi.
...
Ön İşleme: Kategorik verilerin (Kadın/Erkek, Biniş Limanı) One-Hot Encoding ile sayısallaştırılması.
...
Değerlendirme: Doğruluk (Accuracy) skorları, Hata Matrisi (Confusion Matrix) ve Özellik Önem Düzeyleri (Feature Importance).
...
Veri Kaynağı: Seaborn Library (Titanic Dataset)